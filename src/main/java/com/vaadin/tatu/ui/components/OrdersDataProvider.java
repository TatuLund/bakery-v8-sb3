package com.vaadin.tatu.ui.components;
...
import org.vaadin.spring.dataprovider.FilterablePageableDataProvider;
...
public class OrdersDataProvider
    ...
    @Override
    protected Page<Order> fetchFromBackEnd(Query<Order, Object> query,
            Pageable pageable) {
        return orderService.findAnyMatchingAfterDueDate(getOptionalFilter(),
                getOptionalFilterDate(), pageable);
    }
    ...
    protected int sizeInBackEnd(Query<Order, Object> query) {
        return (int) orderService.countAnyMatchingAfterDueDate(
                getOptionalFilter(), getOptionalFilterDate());
    }
